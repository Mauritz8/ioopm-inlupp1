...
/*static void _print_linked_list_helper(list_entry_t *entry)
{
  if (entry == NULL)
  {
    printf("NULL");
  ...
  {
    printf("[value: %d, next->", entry->value);
    _print_linked_list_helper(entry->next);
    printf("]");
  }
}

static void _print_linked_list(ioopm_list_t *list)
{
  puts("");
  if (list != NULL)
  {
    _print_linked_list_helper(list->first);
  }
  puts("");
}*/
